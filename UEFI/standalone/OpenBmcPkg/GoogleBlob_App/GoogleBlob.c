  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    ...
    if (EFI_ERROR(Status)) {
      Print(L"Error! Can't locate SMBIOS table\n");
...
#ifdef DEBUG_SMBIOS_TABLES
  Print(L"Smbios table entry point\n");
  ...
  for (UINT32 id = 0; id < BlobCount; id++) {
    ...
    Print(L"BlobLongWrite SmbiosTableAddress\n");
    ...
    Print(L"BlobLongWrite SmbiosTableEntryPoint\n");
    ...
    Print(L"SMBIOS tables were successfully transmitted to the BMC\n");
  }
